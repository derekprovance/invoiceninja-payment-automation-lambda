...
describe('VenmoPayment', () => {
  describe('name parsing', () => {
    it('parses a simple two-word name', () => {
      const p = new VenmoPayment('John Smith paid $50.00', GATEWAY_ID)
    ...
    it('parses a hyphenated last name', () => {
      const p = new VenmoPayment('Mary-Jane Watson paid $25.00', GATEWAY_ID)
      expect(p.getName()).toBe('Mary-Jane Watson')
    })

    it('parses a name with an apostrophe', () => {
      const p = new VenmoPayment("Patrick O'Brien paid $100.00", GATEWAY_ID)
      expect(p.getName()).toBe("Patrick O'Brien")
    })

    it('parses a three-word name', () => {
      const p = new VenmoPayment('John Paul Jones paid $75.00', GATEWAY_ID)
  ...
  describe('amount parsing', () => {
    it('parses whole dollar amounts', () => {
      const p = new VenmoPayment('Alice Bob paid $100', GATEWAY_ID)
      expect(p.getAmount()).toBe(100)
    })

    it('parses amounts with cents', () => {
      const p = new VenmoPayment('Alice Bob paid $42.99', GATEWAY_ID)
      expect(p.getAmount()).toBe(42.99)
    })

    it('parses amounts with one decimal place', () => {
      const p = new VenmoPayment('Alice Bob paid $9.5', GATEWAY_ID)
  ...
  describe('validation', () => {
    it('throws InvalidPaymentError for $0.00', () => {
      expect(
        () => new VenmoPayment('Alice Bob paid $0.00', GATEWAY_ID),
      ).toThrow(InvalidPaymentError)
    })

    it('throws InvalidPaymentError for $0 (no cents)', () => {
      expect(() => new VenmoPayment('Alice Bob paid $0', GATEWAY_ID)).toThrow(
        InvalidPaymentError,
      )
    })
  })
  ...
  describe('getPaymentId', () => {
    it('returns the gatewayId passed to the constructor', () => {
      const p = new VenmoPayment('Alice Bob paid $10.00', GATEWAY_ID)
      expect(p.getPaymentId()).toBe(GATEWAY_ID)
    })
  })

  describe('parse errors', () => {
    it('throws ParserError when "paid" is missing', () => {
